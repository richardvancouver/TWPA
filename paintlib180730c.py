                ...
                for cpts in centerlinelist:
                    ...
                    for i,pt in enumerate(cpts[1:-1],1):
                        ...
                        if calt_int !=dt_int:
                            ...
                            tr=pya.CplxTrans(1,1*(atan2(dy,dx)/pi*180-180),False,pt.x,pt.y)
                            if dx>0:
                               tr=pya.CplxTrans(1,1*atan2(dy,dx)/pi*180,False,pt.x,pt.y)
                            new_instance=pya.CellInstArray(icell.cell_index(),tr)
                            #if (1*atan2(dy,dx)/pi*180) % 180 == 0:   #added 181220                         
...
########
    def DrawAirbridgeb(self,cell,centerlinelist,newcellname="Airbige"):
        IO.layout.read(self.filename)
        for icell in IO.layout.top_cells():
            if (icell.name == self.insertcellname):
                icell.name=newcellname
...
########
########
    def DrawAirbridgec(self,cell,centerlinelist,newcellname="Airbige"):
        IO.layout.read(self.filename)
        for icell in IO.layout.top_cells():
            if (icell.name == self.insertcellname):
                icell.name=newcellname
...
class Interactive:
    ...
    def _merge_and_draw(outregion,inregion,tr_to=None,cutbool=True):
        ...
        if cutbool:
            ...
            region=outregion-(outregion-inregion)#outregion-inregion#outregion-(outregion-inregion)#
